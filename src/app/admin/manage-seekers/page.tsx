...
import { getJobSeekers, deleteAd } from "@/lib/data";
...
export default function AdminManageSeekersPage() {
  const router = useRouter();
  const { toast } = useToast();
  const { userData, loading: authLoading } = useAuth();

  const [seekers, setSeekers] = useState<Job[]>([]);
  ...
  const fetchSeekers = useCallback(
    async (isLoadMore = false) => {
      ...
      try {
        ...
        setSeekers((prev) => (isLoadMore ? [...prev, ...res.data] : res.data));
        setCursor(res.lastDoc);
        setHasMore(res.data.length === ITEMS_PER_PAGE);
      } catch (error) {
        toast({ variant: "destructive", title: "فشل تحميل إعلانات الباحثين عن عمل" });
      } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await revalidateAll();
      setSeekers((prev) => prev.filter((ad) => ad.id !== adToDelete));
