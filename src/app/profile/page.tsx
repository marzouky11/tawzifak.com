...
import { User, LogOut, ChevronLeft, Loader2, Settings as SettingsIcon, HelpCircle, Shield, FileText, UserPlus, LogIn as LogInIcon, MessageSquare, Bookmark, Flag, PlusCircle } from 'lucide-react';
        ...
        <div className="space-y-6">
            <Card className="overflow-hidden">
                <CardContent className="p-4 flex items-center gap-4">
                    <UserAvatar name={userData.name} color={userData.avatarColor} photoURL={userData.photoURL} className="h-16 w-16 text-2xl shadow-inner flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                        <h2 className="text-xl font-bold truncate" title={userData.name || 'مستخدم'}>
                            {userData.name || 'مستخدم'}
                        </h2>
                        <p className="text-sm text-muted-foreground truncate" title={userData.email}>
                            {userData.email}
                        </p>
                    </div>
                </CardContent>
            ...
            {userData?.isAdmin && (
               <Card>
                ...
                <CardContent className="p-0">
                    <ul className="divide-y divide-border">
                        <SettingItem icon={Shield} label="لوحة مشرف" href="/admin" />
                    </ul>
                </CardContent>
            </Card>
            )}
            ...
            <Card>
                <CardContent className="p-4">
                    <Button variant="destructive" className="w-full active:scale-95 transition-transform" onClick={onLogout}>
        ...
        <div className="space-y-6">
            <Card>
                ...
                <CardContent className="flex flex-col sm:flex-row gap-4 p-4">
                    <Button asChild size="lg" className="flex-1 py-6 active:scale-95 transition-transform">
                        <Link href="/login">
                            <LogInIcon className="mr-2 h-4 w-4" />
                            تسجيل الدخول
                        </Link>
                    </Button>
                    <Button asChild size="lg" variant="outline" className="flex-1 py-6 active:scale-95 transition-transform">
                        <Link href="/signup">
  ...
  return (
    <>
      ...
      <DesktopPageHeader
        ...
        description="تحكم في حسابك وعدّل معلوماتك الشخصية."
      />
