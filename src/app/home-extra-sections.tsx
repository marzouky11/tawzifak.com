...
// علم على مستوى الوحدة (module-level) يبقى في الذاكرة أثناء التنقل بين الصفحات
// داخل الموقع (SPA navigation)، ولا يُعاد ضبطه إلا عند إعادة تحميل الصفحة
// بالكامل أو فتح الموقع من جديد.
let hasStatsCountUpPlayed = false;

const CountUp = ({ end = 0, duration = 2, skipAnimation = false }: { end?: number, duration?: number, skipAnimation?: boolean }) => {
  const [count, setCount] = useState(skipAnimation ? end : 0);
  const ref = useRef<HTMLSpanElement>(null);

  useEffect(() => {
    if (skipAnimation || !end || end === 0) {
      setCount(end || 0);
      return;
    }
...
function StatsSection({ stats }: { stats: { jobs?: number, competitions?: number, immigration?: number, seekers?: number } }) {
  // إذا كانت حركة العد قد اكتملت سابقًا خلال هذه الجلسة، لا نعيد تشغيلها
  // عند التنقل بين الصفحات والعودة إلى الصفحة الرئيسية.
  const skipAnimationRef = useRef(hasStatsCountUpPlayed);

  useEffect(() => {
    hasStatsCountUpPlayed = true;
  }, []);
          ...
          {statItems.map((item) => (
            ...
            >
              ...
              >
                ...
                >
                  <CountUp end={item.count} skipAnimation={skipAnimationRef.current} />
                </div>
              </Card>
            </div>
          ))}
