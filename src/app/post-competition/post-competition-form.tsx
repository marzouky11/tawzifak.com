...
import { 
    ...
    ArrowLeft, ArrowRight, Building, ListOrdered, FileUp, LogIn, Users2, MapPin, Briefcase, Mail, HelpCircle, X
} from 'lucide-react';
...
const stepFields: FieldPath<z.infer<typeof formSchema>>[][] = [
  ['title', 'organizer', 'customOrganizer', 'positionsAvailable', 'competitionType', 'location', 'registrationStartDate', 'deadline', 'competitionDate'],
  ['description', 'availablePositions', 'requirements', 'documentsNeeded', 'competitionStages', 'extraSections', 'howToApply'],
  ['officialLink', 'fileUrl', 'email'],
];
...
const steps = [
    ...
    { id: 3, name: "روابط التقديم", description: "روابط ووسائل التواصل.", icon: LogIn },
];
...
export function PostCompetitionForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const { id } = await postCompetition(dataToSave as Omit<Competition, "createdAt" | "id" | "postedAt">);
      await revalidateAll();
      toast({
  ...
  const stepsContent = [
    <div className="space-y-6" key="step1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField control={form.control} name="registrationStartDate" render={({ field }) => (<FormItem><FormLabelIcon icon={CalendarIcon} label="تاريخ بداية التسجيل (اختياري)" /><FormControl><Input placeholder="مثال: 2024-08-01" {...field} /></FormControl><FormMessage /></FormItem>)} />
