...
const visaTypeOptions: { value: string; label: string }[] = [
  { value: 'work', label: 'تأشيرة عمل' },
  { value: 'study', label: 'تأشيرة دراسة' },
  { value: 'training', label: 'تأشيرة تدريب' },
  { value: 'volunteer', label: 'تأشيرة تطوع' },
  { value: 'job_seeker', label: 'تأشيرة باحث عن عمل' },
  { value: 'working_holiday', label: 'تأشيرة عطلة وعمل (Working Holiday)' },
  { value: 'permanent_residency', label: 'تأشيرة هجرة دائمة' },
  { value: 'family_reunification', label: 'تأشيرة لم الشمل' },
  { value: 'seasonal', label: 'تأشيرة موسمية' },
...
const formSchema = z.object({
  ...
  path: ['phone'],
    ...
    message: "يجب اختيار مجال البرنامج أو إدخال نوع مخصص.",
    path: ["programType"],
});


const stepFields: FieldPath<z.infer<typeof formSchema>>[][] = [
  ['title', 'programType', 'customProgramType', 'visaType', 'targetCountry', 'city', 'targetAudience', 'positionsAvailable', 'salary', 'deadline'],
  ['description', 'availablePositions', 'requirements', 'qualifications', 'experience', 'tasks', 'featuresAndOpportunities', 'extraSections', 'howToApply'],
...
export function PostImmigrationForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const dataToSave: { [key: string]: any } = { ...restOfValues, slug: slugify(values.title) };

      if (customProgramType) {
          dataToSave.programType = customProgramType;
      } else {
          dataToSave.programType = values.programType;
      }

      const { id } = await postImmigration(
        dataToSave as Omit<
          ImmigrationPost,
          "createdAt" | "id" | "postedAt" | "isNew" | "iconName"
        >
      );
      await revalidateAll();
  ...
  const stepsContent = [
    <div className="space-y-6" key="step1">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
            <FormLabelIcon icon={LayoutGrid} label="مجال البرنامج" />
            {(programType || customProgramType) && (
                <Button type="button" variant="ghost" size="sm" className="h-auto p-1 text-xs text-destructive" onClick={() => { form.setValue('programType', undefined); form.setValue('customProgramType', ''); }}>
                    <X className="ml-1 h-3 w-3" />
                    مسح
                </Button>
            )}
        </div>
        <p className="text-sm text-muted-foreground -mt-2">اختر من القائمة أو أدخل مجالاً مخصصًا. لا يمكن اختيار الاثنين معاً.</p>
        <FormField control={form.control} name="programType" render={({ field }) => (
            <FormItem>
                <Select onValueChange={(value) => { field.onChange(value); if (value) form.setValue('customProgramType', ''); }} value={field.value} disabled={!!customProgramType}>
                    <FormControl><SelectTrigger><SelectValue placeholder="اختر مجال البرنامج" /></SelectTrigger></FormControl>
                    <SelectContent>{programTypes.map(p => (<SelectItem key={p.value} value={p.value} className="focus:bg-transparent focus:text-inherit data-[highlighted]:bg-transparent data-[highlighted]:text-inherit data-[state=checked]:bg-accent data-[state=checked]:text-accent-foreground"><div className="flex items-center gap-2"><CategoryIcon name={p.icon} className="w-5 h-5" style={{ color: p.color }} /> {p.label}</div></SelectItem>))}</SelectContent>
                </Select>
                <FormMessage />
            </FormItem>
        )} />
        ...
        <FormField control={form.control} name="customProgramType" render={({ field }) => (
            <FormItem>
                <FormControl><Input placeholder="أدخل مجال برنامج مخصص" {...field} onChange={(e) => { field.onChange(e); if (e.target.value) form.setValue('programType', undefined); }} disabled={!!programType} /></FormControl>
                <FormMessage />
            </FormItem>
        )} />
      </div>
      ...
      <FormField control={form.control} name="visaType" render={({ field }) => (
          <FormItem>
              <FormLabelIcon icon={Plane} label="نوع التأشيرة (اختياري)" />
              <Select onValueChange={field.onChange} value={field.value}>
                  <FormControl><SelectTrigger><SelectValue placeholder="اختر نوع التأشيرة" /></SelectTrigger></FormControl>
                  <SelectContent>{visaTypeOptions.map(opt => (<SelectItem key={opt.value} value={opt.value} className="focus:bg-transparent focus:text-inherit data-[highlighted]:bg-transparent data-[highlighted]:text-inherit data-[state=checked]:bg-accent data-[state=checked]:text-accent-foreground">{opt.label}</SelectItem>))}</SelectContent>
              </Select>
              <FormMessage />
          </FormItem>
      )} />
    ...
    <div className="space-y-6" key="step2">
        ...
        <ExtraSectionsField control={form.control} themeColor={sectionColor} />
