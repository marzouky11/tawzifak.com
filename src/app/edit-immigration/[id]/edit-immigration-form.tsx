...
const formSchema = z.object({
  ...
  path: ['phone'],
    ...
    message: "يجب اختيار مجال البرنامج أو إدخال مجال مخصص.",
    path: ["programType"],
});
...
export function EditImmigrationForm({ post }: EditImmigrationFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      visaType: post?.visaType || undefined,
  ...
  const step1Content = (
    <div className="space-y-6">
        ...
        <FormField control={form.control} name="visaType" render={({ field }) => (
            <FormItem>
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>{visaTypeOptions.map(opt => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}</SelectContent>
                </Select>
                <FormMessage />
            </FormItem>
        )} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="positionsAvailable" render={({ field }) => (<FormItem><FormLabelIcon icon={Users} label="عدد المناصب (اختياري)" /><FormControl><Input placeholder="مثال: 10 مناصب" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
